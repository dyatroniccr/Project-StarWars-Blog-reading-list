...
const UsersDetailedVehicle = () => {
    ...
    useEffect(() => {
        const cargaDatos = async () => {
            let { respuestaJson, response } = await actions.useFetch(`/vehicles/${params.id}`)
            if (response.ok) {
                    ...
                    <div className="row g-0">
                        ...
                        <div className="col-md-8">
                            <div className="card-body">
                                ...
                                <p className="card-text"><small className="text-muted">Name: {vehicle.name} <br />Passengers: {vehicle.passengers} <br />Length: {vehicle.length} <br />Cargo Capacity: {vehicle.cargo_capacity}</small></p>
                            </div>
                        </div>
                    </div>
                </div>
...
export default WithAuth(UsersDetailedVehicle);